...
int main(int argc, char** argv) {
    ...
    for (size_t q = 0; q < states.size(); q++) {
        ...
        for (int d = 0; d < 4; d++) {
            int sx = DX[d], sy = DY[d];
            bool bad = false;
            for (E e : S) if (ex(e) == sx && ey(e) == sy) { bad = true; break; }
            if (bad) continue;
            vector<E> nw;
            auto add = [&](int x, int y, int r) {
                ...
                if (dist <= r && r >= 1) {
                    r -= (r - dist) & 1;
                    if (r <= D && dist <= D) r = 0;       // near-lock dominates
                    nw.push_back(pack(x, y, r));
                } else if (dist <= D) {
                    nw.push_back(pack(x, y, 0));          // kept only while near
                }
            };
    ...
    printf("D=%d m=%d states=%zu  lambda in [%.12f, %.12f]  build %.1fs\n", D, M, n, lo, hi, tb);
}
